...
title: "[Paper review] RePaint: Inpainting using Denoising Diffusion Probabilistic Models"
...
Diffusion models beat GANs on image synthesis 논문 ([Paper review](https://kimjy99.github.io/Paper review/dmbg/))의 slowing down과 비슷하게 resampling도 reverse diffusion의 실행시간을 증가시킨다. Slowing down은 각 denoising step에서 추가된 분산을 줄여 더 작지만 더 많은 resampling step을 적용한다. 그러나 diffusion을 늦추면 설명한 대로 여전히 이미지가 조화되지 않는 문제가 있기 때문에 근본적으로 다른 접근 방식이다. 
