...
##### **Feature Extraction**
  ...
  - CLIP의 이미지 인코더 $\phi_v$ 와 텍스트 인코더 $\phi_t$ 를 사용하여 특징을 추출
    ...
    $$
  - 기존 연구들은 프레임 특징 $\left\{ \mathbf{f}_1, ..., \mathbf{f}_{T'} \right\}$ 를 다양한 방식으로 결합하여 최종 비디오 임베딩을 생성:
    $$
    \mathbf{v} = \psi([\mathbf{f}_1, ..., \mathbf{f}_{T'}], \text{;} 해\mathbf{t})
    $$
...
1. **텍스트 및 비디오 특징 추출**
   - 주어진 텍스트-비디오 쌍 $\left{t, v \right}$ 에 대해 features인 $\mathbf{v}, \mathbf{f}$을 추출:
     $$
...
3. **최적의 텍스트 임베딩 선택**
   - 생성된 $\mathbf{t}_M^s$ 중 **비디오와 가장 유사한 텍스트 임베딩을 선택**:
     $$
     \mathbf{t}^s = \arg\max_{\mathbf{t}_i^s} s(\mathbf{t}_i^s, \mathbf{v}), \quad i = 1, ..., M
     $$
   - **이 방법의 장점**
     - 고정된 $\mathbf{t}$ 가 아닌 **비디오와 가장 적절한 $\mathbf{t}^s$ 를 동적으로 선택**.
     - **다양한 표현을 고려하여 정렬 성능 향상**.

4. **텍스트-비디오 정합성 개선**
   - 기존 방식은 하나의 $\mathbf{t}$ 만 사용하여 검색했으나, **T-MASS는 최적화된 확률적 표현을 선택하여 검색 성능을 향상**.
   - **Text-to-Video 및 Video-to-Text 검색 모두 적용 가능**.
