...
##### **Feature Extraction**
  ...
  - CLIP의 이미지 인코더 $\phi_v$ 와 텍스트 인코더 $\phi_t$ 를 사용하여 특징을 추출
    $$
    \mathbf{f}_i = \phi_v(f_i), \quad i = 1, ..., T'  \; \mathbf{t} = \phi_t(t)
    $$
...
##### **Motivation**
- 기존 연구들은 비디오 임베딩 학습($\mathbf{v}$)(frame sampling protocol, $\phi_v(\cdot)$, $\psi(\cdot)$을 포함)에 집중했으며, 텍스트 표현력 부족 문제를 충분히 해결하지 못함
