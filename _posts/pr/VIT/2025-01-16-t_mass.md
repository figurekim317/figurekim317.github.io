...
- Text-video retrieval은 $t, v \in \mathbb{R}^d$의 공통 임베딩 공간을 학습하는 과정
- 코사인 유사도 등 유사도 측정 함수 $s(t, v)$를 사용하여 텍스트-비디오 간 연관성을 평가
- 손실 함수는 **대칭 교차 엔트로피(Symmetric Cross Entropy)**를 사용하여 관련된 쌍은 가깝게, 무관한 쌍은 멀어지도록 학습
...
##### Feature Extraction
- CLIP 기반의 다중 모달 임베딩을 활용하여 텍스트와 비디오를 공통 공간으로 변환
