...
여기서 각 패치는 $3P^2$ 크기를 가지며, $H$와 $W$는 $P$의 배수라고 가정한다. 각 패치는 학습된 linear projection layer을 통해 $d$-차원의 잠재 공간으로 매핑된다. 또한, 학습된 벡터인 “class token” $\text{CLS}$가 패치 임베딩에 추가되어 Transformer의 입력은 $\mathbb{R}^{(N+1) \times d}$에 속하게 된다.
<br>
...
이 연구에서는 DINO로 self-supervised 방식으로 학습된 Transformers를 사용합니다. Caron et al. [13]에 따르면, 마지막 attention 레이어에서 CLS query의 self-attention을 통해 유의미한 객체 분할을 얻을 수 있습니다. 이를 기반으로 'DINO-seg'라는 baseline을 구축하여 객체 탐지를 수행하며, 이는 공정한 결과를 제공합니다.

그러나 이 접근법은 self-supervised Transformer features의 잠재력을 충분히 활용하지 못한다고 판단했습니다. 이에 따라, 본 연구에서는 패치 간 유사도를 계산하고 이를 활용하여 객체를 더 효과적으로 localize하는 새로운 방법인 LOST를 제안합니다. LOST는 Transformer의 마지막 레이어에서 추출한 패치 키 \( k_p \in \mathbb{R}^d, \, p = 1, \ldots, N \)를 사용하여 단일 이미지의 패치 간 유사도를 계산함으로써 작동합니다.
